...
class LLMVisionClient:
    ...
    def _raise_actionable(exc: Exception) -> None:
        ...
        if "token_expired" in lower or "authentication token has expired" in lower:
            raise RuntimeError(
                "🔑 OpenAI 인증 토큰이 만료되었습니다. 터미널에서 'codex login'으로 "
                "재인증해 주세요. API 키는 명시적으로 manual 인증을 선택한 경우에만 사용합니다."
            ) from exc
        if "401" in lower and "unauthorized" in lower:
            raise RuntimeError(
                "🔑 OpenAI 인증 실패 (401 Unauthorized). 토큰이 만료됐거나 잘못됐습니다. "
                "'codex login'으로 OAuth 세션을 갱신해 주세요."
            ) from exc
        ...
        if "rate_limit" in lower or "429" in lower:
            raise RuntimeError(
                "⏱️ OpenAI rate limit입니다. 잠시 후 다시 시도해 주세요."
            ) from exc
        # 매칭 없음 → 변환 안 함
